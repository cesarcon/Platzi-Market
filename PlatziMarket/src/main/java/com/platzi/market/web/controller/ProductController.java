...
public class ProductController {
	...
	@GetMapping("/all")
	@ApiOperation("Get all supermarket products") /**Anotacion para documentar swagger */
	@ApiResponse(code = 200, message = "Ok") /**Anotacion para documentar swagger */
	public ResponseEntity<List<Product>> getAll(){
		return new ResponseEntity<>(productService.getAll(), HttpStatus.OK);
	}

	@GetMapping("/{id}")
	@ApiOperation("Search a product by ID") /**Anotacion para documentar swagger */
	@ApiResponses({
			@ApiResponse(code = 200, message = "Ok"), /**Anotacion para documentar swagger */
			@ApiResponse(code = 404, message = "Product not fount") /**Anotacion para documentar swagger */
	})
